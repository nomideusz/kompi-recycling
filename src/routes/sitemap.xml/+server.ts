...
function renderUrl(loc: string, changefreq: string, priority: string): string {
...
export const GET: RequestHandler = async () => {
	...
	const urls: string[] = [];
	urls.push(renderUrl(BASE, 'daily', '1.0'));

	for (const p of points) {
		urls.push(renderUrl(`${BASE}/punkt/${p.slug}`, 'weekly', '0.7'));
	}
